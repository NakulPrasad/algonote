...
**AlgoNote AI** is your ultimate note-first study companion for Data Structures and Algorithms. It turns static problem-solving notes into interactive learning tools with active recall quizzes, mock interview simulators, timed grill modes, structural linting, customizable templates, and spaced repetition revision tracking.
...
🔗 **VS Code Marketplace:** [https://marketplace.visualstudio.com/items?itemName=NakulMahato.algonote](https://marketplace.visualstudio.com/items?itemName=NakulMahato.algonote)  
🌐 **Website & Live Demo:** [https://nakulprasad.github.io/algonote/](https://nakulprasad.github.io/algonote/)  
📦 **Download Latest Release:** [GitHub Release v0.1.20](https://github.com/NakulPrasad/algonote/releases/tag/v0.1.20)
...
2. **Code Verification:** Checks that code inside the implementation block is syntactically sound and ready before quizzing you on it.

---

## ✨ Features at a Glance

### 1. 🔍 Pre-flight Note & Code Validation
...
### 2. 🧩 Customizable Note Templates (`TEMPLATE.md`)
Full flexibility to customize how your problem notes are formatted!
- Edit `TEMPLATE.md` in your workspace root or `.algonote/template.md`.
- Customize language blocks (Java, Python, C++, Go), change section layouts, or add personal checklists.
- Dynamic placeholders supported: `${title}`, `${difficulty}`, `${topic}`, `${link}`, `${description}`, `${examples}`, `${intuition}`, `${starterCode}`, `${timeComplexity}`, `${spaceComplexity}`, `${edgeCases}`, `${visualization}`.

### 3. 🧪 Code-Blank MCQs & Active Recall
...
### 4. 💬 Copilot Chat Participant (`@algo`)
...
### 6. 📊 Revision Dashboard (Spaced Repetition)
...
### 7. ⚡ Smart Inline Completions & Snippets
...
```json
{
  // Custom path to your markdown template (optional)
  "algonote.customTemplatePath": "TEMPLATE.md",
  ...
  // Select where Quiz panel launches: "editor-one" | "editor-two" | "browser"
  "algonote.quizLocation": "editor-one"
}
```
...
|:---|:---|:---|
| **AlgoNote: Edit / Customize Note Template** | `algonote.customizeTemplate` | Opens `TEMPLATE.md` to edit your template structure |
...
```powershell
code --install-extension "algonote-0.1.20.vsix"
```
